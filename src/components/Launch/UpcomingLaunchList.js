...
const UpcomingLaunchList = ()=>{
  ...
  const upcomingLaunchData = useSelector((state)=> state.upcomingLaunches.upcomingLaunches);
  ...
  useEffect(()=>{
   ...
   const currentData = upcomingLaunchData.slice(indexOfFirstData,indexOfLastData-1);
    ...
    return(
      <Fragment>
        <section className={classes.list}>
          ...
          <Pagination dataPerPage={dataPerPage} totalData={upcomingLaunchData.length} paginate={paginate}/>
        </section>
      </Fragment>
    )
}
