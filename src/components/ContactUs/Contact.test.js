...
describe("login", () => {
  ...
  test("company input should accept text", () => {
    ...
    fireEvent.change(companyInputNode, {
      target: { value: "brainstorm force" }
    });
  ...
  test("description input should accept text", () => {
    render(<Contact />);
    const descriptionInputNode = screen.getByLabelText(
      "Describe about your proposal"
    );
    expect(descriptionInputNode.value).toMatch("");
    fireEvent.change(descriptionInputNode, {
      target: { value: "this is testing" }
    });
